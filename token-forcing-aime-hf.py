...
def load_and_preprocess_aime_dataset(model_name, split="train"):
    ...
    Returns a tuple (processed_data, tokenizer).
    """
    try:
        ...
        for example in dataset:
            ...
            # Create prompt (without forced tokens).
            input_prompt = (
...
def extract_boxed_answer(solution_text):
    """
    Extracts the answer that appears inside \\boxed{...} in the solution text.
    """
    pattern = r'\\boxed\{([^{}]+)\}'
...
# ----------------------------
# 2. Hugging Face LogitsProcessor for Forced Tokens
# ----------------------------
class MultiStepForceTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        current_step = input_ids.shape[1]  # current total sequence length (prompt + generated tokens)
        if current_step in self.forced_tokens:
            ...
            forced_scores[:, token_id] = 0  # Force the desired token.
            return forced_scores
        return scores

# ----------------------------
# 3. Model loading, Accelerator, and setup
# ----------------------------
temperature = 0.6
top_p = 0.95
max_new_tokens = 16000  # Total new tokens to generate per example
num_examples = 100      # Process first 100 examples (for demonstration)
...
# Load model once using device_map="auto" (uses available GPUs)
model = AutoModelForCausalLM.from_pretrained(model_name)
model = accelerator.prepare(model)

# Create a directory to save generation results if it doesn't exist.
...
# ----------------------------
# We wish to force two tokens: newline and </think>
forced_token1 = "\n"
forced_token2 = "</think>"
...
if len(forced_token1_ids) != 1:
    raise ValueError("Forced newline token encoded as multiple tokens; adjust your approach.")
if len(forced_token2_ids) != 1:
    raise ValueError("Forced '</think>' token encoded as multiple tokens; adjust your approach.")

forced_token1_id = forced_token1_ids[0]
forced_token2_id = forced_token2_ids[0]

# ----------------------------
# 5. Loop over forced offsets and process in batches
# ----------------------------
# List of forced offsets (number of tokens after the prompt) where forced tokens will be inserted.
forced_offsets = [500, 1000, 2000, 4000, 6000, 8000, 9000, 10000, 12000, 14000]

# Dictionaries to store aggregated metrics and generations for each forced offset.
results_dict = {}  # Key: forced_offset, Value: list of generation strings (for successfully processed examples)
metrics_dict = {}  # Key: forced_offset, Value: (accuracy, avg_length)

# We'll use the underlying generate function.
...
for force_offset in forced_offsets:
    ...
    all_total = 0
    all_lengths = []
    gen_texts = []  # store generation outputs for this forced offset
    ...
    for i in tqdm(range(0, num_total, batch_size), desc=f"Forced offset {force_offset}"):
        ...
        # Determine batch prompt length (assume uniform length in batch)
        batch_prompt_length = input_ids_batch.shape[1]
        
        # Build forced_tokens dict for this batch.
        forced_tokens = {
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                print(f"OOM encountered at forced offset {force_offset}, batch {i//batch_size}; skipping this batch.")
    ...
    # Only compute metrics if at least one example was processed.
    if all_total > 0:
    ...
    print(f"Forced offset: {force_offset} -> Accuracy: {acc if acc is not None else 'N/A'}%, "
          f"Avg generated length: {avg_length if avg_length is not None else 'N/A'} tokens")
    
    # Save intermediate results after each forced offset.
...
# Filter out forced offsets with no processed examples.
valid_offsets = [fo for fo, acc in zip(forced_offsets, accuracy_list) if acc is not None]
valid_accuracy = [acc for acc in accuracy_list if acc is not None]
valid_length = [l for l in length_list if l is not None]
